 ...
 * @param {!ngeox.number} ngeoNumber
 * @param {!ngeox.unitPrefix} ngeoUnitPrefix
 * @param {!ngeox.numberCoordinates} ngeoNumberCoordinates
 * @param {number} ngeoMeasureDecimals
 * @param {number} ngeoMeasurePrecision
 * @ngdoc service
 * @ngname ngeoFeatureHelper
 * @ngInject
 */
ngeo.FeatureHelper = function($injector, $filter,
    ngeoNumber, ngeoUnitPrefix, ngeoNumberCoordinates,
    ngeoMeasureDecimals, ngeoMeasurePrecision) {
   ...
   * @type {!ngeox.number}
   */
  this.numberFormat_ = ngeoNumber;

  /**
   * @type {!ngeox.unitPrefix}
   */
  this.unitPrefixFormat_ = ngeoUnitPrefix;

  /**
   * @type {!ngeox.numberCoordinates}
   */
  this.ngeoNumberCoordinates_ = ngeoNumberCoordinates;
...
ngeo.FeatureHelper.prototype.getPolygonStyle_ = function(feature) {
  ...
  const azimut = goog.asserts.assertNumber(feature.get(ngeo.FeatureProperties.AZIMUT));
...
ngeo.FeatureHelper.prototype.getOpacityProperty = function(feature) {
  const opacityStr = goog.asserts.assertString(feature.get(ngeo.FeatureProperties.OPACITY));
  const opacity = opacityStr !== undefined ? +opacityStr : 1;
...
ngeo.FeatureHelper.prototype.getSizeProperty = function(feature) {
  const size = +(goog.asserts.assertString(feature.get(ngeo.FeatureProperties.SIZE)));
...
ngeo.FeatureHelper.prototype.getStrokeProperty = function(feature) {
  const stroke = +(goog.asserts.assertString(feature.get(ngeo.FeatureProperties.STROKE)));
